...
def _seed_db(path: str, messages: list) -> None:
    """Create the messages and messages_fts tables, then insert the given records."""
    ...
    for i, msg in enumerate(messages):
        ...
        # Insert a matching messages_fts row for FTS search
        conn.execute(
            "INSERT INTO messages_fts (message_id, subject, body_text, sender_text) VALUES (?,?,?,?)",
            (message_id, msg["subject"] or "", "", ""),
        )
    conn.commit()
    conn.close()
...
# Feature: messages-fts-view, Property 5: List response envelope invariant
...
def test_property_1_pagination_total_consistency(messages, page, page_size):
    ...
    try:
        ...
        # Property 5: envelope keys must all be present
        for key in ("messages", "total", "page", "page_size"):
            assert key in data, (
                f"Response envelope is missing key {key!r}. Keys present: {list(data.keys())!r}"
            )
        ...
        assert len(returned) <= page_size, (
            f"len(messages)={len(returned)} > page_size={page_size}"
        )
        assert len(returned) <= total, (
            f"len(messages)={len(returned)} > total={total}"
        )
...
def test_property_3_search_filter_soundness(messages, query):
    ...
    try:
        ...
        for msg in data["messages"]:
            ...
            # The API also matches on exact message_id equality (m.message_id = ?)
            message_id_match = msg["message_id"] == query
...
def test_property_7_labels_endpoint_completeness(messages):
    ...
    try:
        ...
        assert resp.status_code == 200
        returned_labels_raw = resp.get_json()
        assert isinstance(returned_labels_raw, list)

        # /api/labels returns a list of {"label": ..., "label_type": ...} dicts
        returned_label_strings = [item["label"] for item in returned_labels_raw]
        ...
        for lbl in expected_labels:
            assert lbl in returned_label_strings, (
                f"Label {lbl!r} from a non-deleted message is missing from /api/labels"
            )

        # Response must be sorted alphabetically (by label string)
        assert returned_label_strings == sorted(returned_label_strings), (
            f"Labels are not sorted alphabetically: {returned_label_strings!r}"
        )
    finally:
        _cleanup(db_path)
...
# ---------------------------------------------------------------------------
# Property 1 (messages-fts-view): View round-trip consistency
# Validates: Requirements 1.2, 1.3, 6.5
# ---------------------------------------------------------------------------

CREATE_MESSAGES_MINIMAL_SQL = """
CREATE TABLE messages (
    message_id    TEXT PRIMARY KEY,
    thread_id     TEXT,
    sender        TEXT,
...
@given(
    pairs=st.lists(
        st.tuples(safe_text, safe_text),
        min_size=1,
        max_size=20,
    )
)
@settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow], deadline=None)
def test_property_view_round_trip_consistency(pairs):
    # Feature: messages-fts-view, Property 1: messages table round-trip consistency
    # (messages_view removed — queries go directly to messages table)
    assume(len({mid for mid, _ in pairs}) == len(pairs))

    conn = sqlite3.connect(":memory:")
    try:
        conn.execute(CREATE_MESSAGES_MINIMAL_SQL)

        for message_id, body_text in pairs:
            conn.execute(
                "INSERT INTO messages VALUES (?,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL)",
                (message_id,),
            )
        conn.commit()
        ...
        expected_ids = {mid for mid, _ in pairs}
        assert len(rows) == len(pairs), (
            f"Expected {len(pairs)} rows from messages, got {len(rows)}"
        )
        for (row_id,) in rows:
            assert row_id in expected_ids, (
                f"Unexpected message_id {row_id!r} in messages"
            )
    finally:
        conn.close()
...
# ---------------------------------------------------------------------------
# Property 3 (messages-fts-view): Detail response includes body_text
# Validates: Requirements 3.2, 3.3, 5.3
# ---------------------------------------------------------------------------

@given(messages=st.lists(message_strategy(), min_size=1, max_size=20))
@settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow], deadline=None)
def test_property_detail_includes_body_text(messages):
    # Feature: messages-fts-view, Property 3: Detail response includes body field
    # (body_text removed — plain text is now derived from raw and returned as 'body')
    ...
    try:
        for i in range(len(messages)):
            ...
            assert resp.status_code == 200, (
                f"Expected 200 for message_id={message_id!r}, got {resp.status_code}"
            )
            data = resp.get_json()
            assert "body" in data, (
                f"Response for message_id={message_id!r} is missing 'body' key. "
                f"Keys present: {list(data.keys())!r}"
            )
            # Seeded messages have no raw, so body is None
            assert data["body"] is None, (
                f"Expected body to be None for message_id={message_id!r}, "
                f"got {data['body']!r}"
            )
    finally:
        _cleanup(db_path)
...
# ---------------------------------------------------------------------------
# Property 4 (messages-fts-view): Existing response fields preserved
# Validates: Requirements 5.2, 5.3
# ---------------------------------------------------------------------------

@given(messages=st.lists(message_strategy(), min_size=1, max_size=20))
@settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow], deadline=None)
def test_property_existing_fields_preserved(messages):
    # Feature: messages-fts-view, Property 4: Existing response fields preserved
    assume(len(messages) > 0)
    client, db_path = _make_client(messages)
    try:
        for i in range(len(messages)):
            ...
            # Assert all pre-existing fields are present
            required_fields = [
                "message_id", "thread_id", "sender", "recipients", "labels",
                "subject", "timestamp", "is_read", "is_outgoing", "is_deleted",
                "body", "body_html", "attachments",
            ]
            for field in required_fields:
                assert field in data, (
                    f"Response for message_id={message_id!r} is missing field {field!r}. "
                    f"Keys present: {list(data.keys())!r}"
                )

            # Assert correct types for structured fields
            assert isinstance(data["sender"], dict), (
                f"Expected 'sender' to be dict, got {type(data['sender'])!r}"
            )
            assert isinstance(data["recipients"], dict), (
                f"Expected 'recipients' to be dict, got {type(data['recipients'])!r}"
            )
            assert isinstance(data["labels"], list), (
                f"Expected 'labels' to be list, got {type(data['labels'])!r}"
            )
            assert isinstance(data["attachments"], list), (
                f"Expected 'attachments' to be list, got {type(data['attachments'])!r}"
            )
            assert isinstance(data["is_read"], bool), (
                f"Expected 'is_read' to be bool, got {type(data['is_read'])!r}"
            )
            assert isinstance(data["is_outgoing"], bool), (
                f"Expected 'is_outgoing' to be bool, got {type(data['is_outgoing'])!r}"
            )
            assert isinstance(data["is_deleted"], bool), (
                f"Expected 'is_deleted' to be bool, got {type(data['is_deleted'])!r}"
            )
    finally:
        _cleanup(db_path)
