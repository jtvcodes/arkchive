...
class TestEmptyRecipientsBaselineBehavior:
    ...
    def test_body_returned_unchanged(self):
        """API returns body derived from raw (NULL → None).
        ...
        try:
            ...
            assert resp.status_code == 200
            data = resp.get_json()
            # body is derived from raw; raw is NULL so body is None
            assert data.get("body") is None, (
                f"Expected body=None (derived from NULL raw) but got {data.get('body')!r}"
            )
        finally:
            _cleanup(db_path)
...
def test_property_3_5_subject_date_labels_body_unaffected(msg):
    ...
    **Validates: Requirements 3.5**
    ...
    try:
        ...
        # Body: derived from raw (which is NULL), so always None
        assert data.get("body") is None, (
            f"Expected body=None (derived from NULL raw) but got {data.get('body')!r}. "
            "Body is now derived from raw, not stored."
        )
    finally:
        _cleanup(db_path)
...
def _seed_preservation_db(path: str) -> None:
    ...
    # Populate FTS for each seeded row
    for row in PRESERVATION_SEED_ROWS:
