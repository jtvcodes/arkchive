...
DETAIL_FIELDS = SUMMARY_FIELDS + ("recipients", "raw")
...
def _sanitize_fts_query(q: str) -> str:
    """Escape FTS5 special characters so the input is treated as a literal phrase.

    Wraps the query in double-quotes after escaping any internal double-quotes
    by doubling them (FTS5 escape convention).
    """
    escaped = q.replace('"', '""')
    return f'"{escaped}"'
...
def list_messages() -> flask.Response:
    ...
    if sort_dir not in ("asc", "desc"):
        sort_dir = "desc"
    order_sql = f"ORDER BY m.timestamp {sort_dir.upper()}"

    db = get_db()

    try:
        # Build filter conditions on the messages table (alias m)
        conditions = []
        params: list = []
        ...
        if is_outgoing is not None:
            conditions.append("m.is_outgoing = ?")
            params.append(1 if is_outgoing else 0)

        if q:
            # FTS5 search: match against messages_fts OR exact message_id
            sanitized_q = _sanitize_fts_query(q)
            fts_condition = (
                "(m.message_id IN (SELECT message_id FROM messages_fts WHERE messages_fts MATCH ?)"
                " OR m.message_id = ?)"
            )
            conditions.append(fts_condition)
            params = [sanitized_q, q] + params

        where_sql = ("WHERE " + " AND ".join(conditions)) if conditions else ""
        fields_sql = ", ".join(f"m.{f}" for f in SUMMARY_FIELDS) + ", m.raw"

        # Query the messages table directly (not the view) for the list —
        # the view's LEFT JOIN with messages_fts is expensive and body_text
        # is not needed for the summary list.
        total = db.execute(
            f"SELECT COUNT(*) FROM messages m {where_sql}",
            params,
        ).fetchone()[0]

        offset = (page - 1) * page_size
        rows = db.execute(
            f"SELECT {fields_sql} FROM messages m {where_sql} {order_sql} LIMIT ? OFFSET ?",
...
def get_message(message_id: str) -> flask.Response:
    ...
    # Derive plain-text body from raw for the UI text toggle
    try:
        msg["body"] = extract_plain_from_raw(raw or "")
    except Exception as exc:
        logger.error(f"Failed to extract body for message {message_id}: {exc}")
        msg["body"] = None
